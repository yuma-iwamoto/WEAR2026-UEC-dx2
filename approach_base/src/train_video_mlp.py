...
from approach_base.src.video_only_common import WindowedVideoDataset, TestVideoDataset, balanced_class_weights, build_video_mlp_feature_vector, build_video_temporal_features, compute_metrics, load_video_records, predict_probabilities, save_eval_outputs, save_submission, seed_everything, train_one_epoch, evaluate_probabilities_and_loss
...
class VideoMLP(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, second_hidden_dim: int, dropout: float):
        super().__init__()
    ...
    def forward(self, video: torch.Tensor) -> torch.Tensor:
        ...
        if video.dim() > 2:
            video = video.reshape(video.size(0), -1)
        if video.dim() != 2:
            raise ValueError(f'Expected 2D feature tensor, got shape={tuple(video.shape)}')
        return self.head(video)


class VideoFeatureDataset(Dataset):
    def __init__(self, base_dataset: Dataset, video_feature_mode: str = 'raw'):
        self.base_dataset = base_dataset
        self.video_feature_mode = video_feature_mode
        self.meta = getattr(base_dataset, 'meta', None)
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        temporal_video = build_video_temporal_features(sample['video'].numpy(), self.video_feature_mode)
        sample['video'] = torch.tensor(build_video_mlp_feature_vector(temporal_video), dtype=torch.float32)
        return sample


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Train a simple video-only MLP baseline.')
    parser.add_argument('--root', type=Path, default=Path('/workspace/input/3rd-wear-dataset-challenge-hasca-2026'))
    parser.add_argument('--output-dir', type=Path, default=Path('/workspace/approach_base/output/video_mlp'))
    ...
    parser.add_argument('--second-hidden-dim', type=int, default=256)
...
def main() -> None:
    ...
    print(f'start video_mlp device={device.type} input_mode=vi_mean_std_delta_scalar window_label_mode={args.window_label_mode} window_size={args.window_size} stride={args.stride} video_window_size={args.video_window_size} video_frame_selection={args.video_frame_selection} video_feature_mode={args.video_feature_mode}')
    ...
    train_loader = DataLoader(VideoFeatureDataset(train_ds, args.video_feature_mode), batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=device.type == 'cuda')
    val_loader = DataLoader(VideoFeatureDataset(val_ds, args.video_feature_mode), batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=device.type == 'cuda')

    input_dim = int(build_video_mlp_feature_vector(build_video_temporal_features(train_ds.samples[0]['video'], args.video_feature_mode)).shape[0])
    print(f'mlp_feature_dim={input_dim}')
    model = VideoMLP(input_dim=input_dim, hidden_dim=args.hidden_dim, second_hidden_dim=args.second_hidden_dim, dropout=args.dropout).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics['macro_f1'] > best_score:
            ...
            bad_epochs = 0
            torch.save({'model_state_dict': best_state, 'args': vars(args), 'best_epoch': best_epoch, 'best_macro_f1': best_score}, args.output_dir / 'video_mlp_best.pt')
    ...
    _, train_prob, y_train = predict_probabilities(model, DataLoader(VideoFeatureDataset(train_ds, args.video_feature_mode), batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=device.type == 'cuda'), device)
    ...
    if args.predict_test:
        test_ds = VideoFeatureDataset(TestVideoDataset(args.root, video_frame_selection=args.video_frame_selection, video_feature_mode='raw'), args.video_feature_mode)
        ...
        output_path = args.output_dir / 'submission_mlp_video.csv'
    ...
    save_run_metadata(
        ...
        outputs={
            ...
            "checkpoint": args.output_dir / "video_mlp_best.pt",
